...
def _build_online(wb,name,dims,framework):
    ...
    _title(ws,f"{name}  —  online psychometric",
        f"SCORING KEY (constant): Score = roundup(Sten/2)  ->  1-2=1 · 3-4=2 · 5-6=3 · 7-8=4 · 9-10=5",
        "E")
    ws["A4"]=("Assessor pastes stens in the blue block on the right; consultant picks the mapped "
              "dimension per behaviour; Sten & Score fill automatically.")
    ws["A4"].font=F_IT; ws.merge_cells("A4:E4"); ws["A4"].alignment=A_WV
    for c,h in zip(["A","B","C","D","E"],["Competency","Behaviour",f"Mapped {name} Dimension","Sten","Score (1-5)"]):
        ws[f"{c}{HR}"]=h
    _hdr(ws,HR,["A","B","C","D","E"])
    # constant paste-stens block on the right (cols H/I)
    ws[f"H{HR}"]=f"{name} Dimension"; ws[f"I{HR}"]="Paste Sten"; _hdr(ws,HR,["H","I"])
    for i,dim in enumerate(dims):
        r=FB+i
        ws[f"H{r}"]=dim; ws[f"H{r}"].font=F_N; ws[f"H{r}"].border=BD; ws[f"H{r}"].fill=C_DIMBG
        ws[f"I{r}"]=""; ws[f"I{r}"].fill=C_PASTE; ws[f"I{r}"].border=BD; ws[f"I{r}"].alignment=A_CTR
    dim_last=FB+len(dims)-1
    for i,(comp,beh) in enumerate(framework):
        r=FB+i
        ws[f"A{r}"]=comp; ws[f"B{r}"]=beh
        ws[f"C{r}"]=""; ws[f"C{r}"].fill=C_IN
        ws[f"D{r}"]=f'=IFERROR(INDEX($I${FB}:$I${dim_last},MATCH(C{r},$H${FB}:$H${dim_last},0)),"")'; ws[f"D{r}"].fill=C_LOCK
        ws[f"E{r}"]=f'=IF(D{r}="","",ROUNDUP(D{r}/2,0))'; ws[f"E{r}"].fill=C_LOCK
        _cellsetup(ws,r,["A","B","C","D","E"])
        ws[f"D{r}"].alignment=A_CTR; ws[f"E{r}"].alignment=A_CTR
    dv=DataValidation(type="list",formula1=f"=$H${FB}:$H${dim_last}",allow_blank=True)
    ws.add_data_validation(dv); dv.add(f"C{FB}:C{FB+len(framework)-1}")
    for c,w in zip(["A","B","C","D","E","F","G","H","I"],[20,44,24,7,9,3,3,24,11]):
