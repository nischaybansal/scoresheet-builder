...
CP_HEADER_ROW = 2              # consultant pastes with headers landing in row 2
CP_FIRST_DATA_ROW = 3
CP_LAST_COL = "CZ"
...
def _build_online(wb,name,dims,framework,mq_mode="5-point"):
...
def _build_consultant_paste(wb):
    ...
    ws["A1"]=("Paste the online export here so its HEADER row lands in row 2 and participants start "
              "in row 3. Column order does not matter - Onlines Data picks each column up by its "
              "header name, so just make sure the header names match Onlines Data (row 3), including "
              "First Name, Last Name, Name, Email, Status.")
    ws["A1"].font=F_IT; ws.merge_cells("A1:L1"); ws["A1"].alignment=A_WV
    ws.row_dimensions[1].height=42
    for r in range(CP_HEADER_ROW,CP_FIRST_DATA_ROW+2):
        for c in range(1,13):
...
def _build_onlines_data(wb,sjt_variant=DEFAULT_SJT_VARIANT,selected_online=None):
    ...
    all_groups={"MQ":("MQ.M5 Profile",MQ_DIMS),
                "OPQ":("OPQ32 Profile",OPQ_DIMS+["Consistency Measure"]),
    ...
    headers=ident+[ws.cell(3,c).value for c in range(6,col_idx)]
    for cidx,h in enumerate(headers,start=1):
        col=openpyxl.utils.get_column_letter(cidx)
        hcell=ws.cell(OD_HELPER_ROW,cidx)
        hcell.value=f'=IFERROR(MATCH("{h}",{cp_head},0),"")'
        hcell.font=F_IT
...
def _build_di(wb,selected,framework,tool_comps,mq_mode="5-point"):
    ...
    layout={"tool_cols":tool_cols,"cols":cols,"groups":groups,"start":start,
            "beh_col":beh_col,"comp_rating_col":comp_rating_col}
...
def build_workbook(framework, selected_tools, sjt_variant=DEFAULT_SJT_VARIANT, tool_comps=None,
    ...
    for name in selected:
        ...
        if spec["kind"]=="online":
            ...
            _build_online(wb,name,dims,filt,mq_mode)
        elif spec["kind"]=="bars": _build_bars(wb,name,filt)
        elif spec["kind"]=="bei":  _build_bei(wb,name,filt)
    _build_tchart(wb,framework)
    di_layout=_build_di(wb,selected,framework,tc,mq_mode)
    _build_integration(wb,selected,framework,tc,di_layout)
