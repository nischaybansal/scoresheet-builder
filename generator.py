...
TRAIT_SLOTS = 12   # max mapped traits per competency on online tools
...
def _build_online(wb,name,dims,framework,mq_mode="5-point",online_paste=True,meta=None):
    ...
    for comp,behs,gs,ge in geo:
        for k in range(ge-gs+1):
            ...
            if k<len(behs):
                ...
                if has_level:
                    ws[f"{level_col}{r}"]=(meta.get("levels") or {}).get((comp,behs[k]),"")
            if k<slots:
        ...
        # Merge common Theme / Competency / Level / Average cells across the competency block
        merge_cols = [comp_col, avg]
        if has_theme:
            merge_cols.append(theme_col)
        if has_level:
            merge_cols.append(level_col)

        for mc in merge_cols:
            if ge > gs:
                ws.merge_cells(f"{mc}{gs}:{mc}{ge}")
            for rr in range(gs, ge + 1):
                ws[f"{mc}{rr}"].border = BD
                ws[f"{mc}{rr}"].alignment = Alignment(
                    horizontal="center",
                    vertical="center",
                    wrap_text=True
                )
    ...
    if has_dir:
        ...
        # Highlight OPQ stens red when Direction is Negative
        ws.conditional_formatting.add(
            f"{sten_col}{ONLINE_FB}:{sten_col}{last}",
            FormulaRule(
                formula=[f'{dir_col}{ONLINE_FB}="Negative"'],
                fill=C_BAD
            )
        )
...
def _build_di(wb,selected,framework,tool_comps,mq_mode="5-point",online_paste=True,
    ...
    for comp,items in groupby(framework,key=lambda x:x[0]):
        ...
        for (c,beh) in items:
            ...
            if has_level:
                di[f"{level_col}{di_row}"]=(meta.get("levels") or {}).get((comp,beh),"")
...
def build_template():
    ...
    ws["A2"]=("One row per behaviour. Repeat the competency on each of its behaviour rows, or "
              "leave it blank to inherit the one above. Theme and Level are optional - leave "
              "them empty and they are left out; rename them and your wording is used.")
