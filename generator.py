...
def _build_mq_standalone(wb, dims):
    ...
    ws["A4"] = ("Not mapped to competencies - this profile stands on its own and does not feed "
                "Detailed Integration. Stens come from the participant selected in Detailed "
                "Integration. This sheet is protected; only the rubric cells are editable.")
...
def build_workbook(framework, selected_tools, sjt_variant=DEFAULT_SJT_VARIANT, tool_comps=None,
    ...
    for name in selected:
        ...
        if mq_standalone and name=="MQ":
            _build_mq_standalone(wb, MQ_DIMS)
            continue
    ...
    di_layout=_build_di(wb,mapped,framework,tc,mq_mode)
