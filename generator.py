...
def _build_online(wb,name,dims,framework):
    ...
    ws["A4"]=("Scores flow automatically from the 'Onlines Data' sheet for the participant selected in "
              ...
              "Direction (Positive by default). This sheet is protected - only mapping, direction and rubric "
              "cells are editable.")
    ...
    _hdr(ws,HR,cols)
    # dimension list (dropdown source), locked
    ws[f"N{HR}"]=f"{name} Dimensions"; _hdr(ws,HR,["N"])
    ...
    for comp,behs,gs,ge in geo:
        for k in range(ge-gs+1):
            ...
            if k<TRAIT_SLOTS:
                ...
                ws[f"J{r}"]=(f'=IF(OR({EMAIL_REF}="",C{r}=""),"",'
                             f'IFERROR(INDEX({od_data},MATCH({EMAIL_REF},{od_email},0),MATCH(C{r},{od_names},0)),""))')
                ws[f"J{r}"].fill=C_LOCK
                ws[f"K{r}"]=(f'=IF(J{r}="","",IF(D{r}="Negative",6-ROUNDUP(J{r}/2,0),ROUNDUP(J{r}/2,0)))')
                ws[f"K{r}"].fill=C_LOCK
                # editable cells stay editable under protection
                for cc in ["C","D","E","F","G","H","I"]:
    ...
    for c,w in zip(["A","B","C","D","E","F","G","H","I","J","K","L","M","N"],
                   [20,38,22,10,17,17,17,17,17,10,9,12,3,24]):
...
def _build_di(wb,selected,framework):
    ...
    for c,t in zip(cols[4:],tool_cols):
        di[f"{c}{H-1}"]=TOOLS[t]["family"].upper(); di[f"{c}{H-1}"].font=F_TAG; di[f"{c}{H-1}"].alignment=A_CTR
    # Consistency Measure (OPQ validity indicator) just above the OPQ heading
    if "OPQ" in tool_cols:
    ...
    start=H+1; idx=0; di_row=start; groups=[]
