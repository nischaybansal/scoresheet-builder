...
def _build_consultant_paste(wb):
    ...
    ws["A1"]=("Paste the SHL online export here so its HEADER row lands in row 2 and the first "
              "participant lands in row 3. Column order does not matter and extra columns are "
              "ignored - Onlines Data finds each column by its header text. Keep the export's own "
              "headers (e.g. 'Persuasive (RP1)-Sten', 'Team-STEN') and include Project Name, "
              "First Name, Last Name, Email, Completed Date and Status.")
