...
VERIFY_DIMS = ["Deductive Reasoning","Inductive Reasoning","Numerical Reasoning"]
VERIFY_GROUP = "G+ Package"          # the export's group label for the Verify block
VERIFY_METRICS = {"Sten":"STEN","Percentile":"PERCENTILE"}
DEFAULT_VERIFY_METRIC = "Sten"
# Percentile -> 1-5 default banding (even fifths). Written onto the Verify sheet as an
# editable table, so the consultant can re-cut the bands per project.
PCT_BANDS_DEFAULT = [(1,20,1),(21,40,2),(41,60,3),(61,80,4),(81,100,5)]
...
def raw_header(trait, verify_metric=DEFAULT_VERIFY_METRIC):
    ...
    for v in SJT_VARIANTS.values():
        if trait in v["dims"]:
            return f"{trait}-STEN"
    if trait in VERIFY_DIMS:
        # e.g. "VAT G Plus - Deductive Reasoning Score-STEN"
        return f"VAT G Plus - {trait} Score-{VERIFY_METRICS[verify_metric]}"
...
C_OK=PatternFill("solid",fgColor="C6EFCE")     # everything matched
...
def _merge_if_uniform(ws,col,gs,ge,values):
    ...
    Blank cells are treated as "same as the row above", not as a second value - clients
    normally type the definition once on the competency's first row and leave the rest
    empty, and that must still merge. Returns True if it merged."""
    if not col or ge<=gs:
        return False
    ...
    if len(seen)>1:
        return False
    if seen:                       # put the value on the anchor, wherever it was typed
        ws[f"{col}{gs}"]=seen.pop()
...
def _online_block(ws,name,dims,online_paste,pct_mode=False):
    ...
    _m="Percentile" if pct_mode else "Sten"
    ws[f"A{OB_STEN_ROW}"]=(f"{_m} (auto)" if online_paste else f"Paste {_m} here")
...
def _build_online(wb,name,dims,framework,mq_mode="5-point",online_paste=True,meta=None,
                  verify_metric=DEFAULT_VERIFY_METRIC):
    from openpyxl.styles import Protection
    has_dir = (name=="OPQ")
    low_high = (name=="MQ" and mq_mode=="low-high")
    pct_mode = (name=="Verify" and verify_metric=="Percentile")
    ...
    key=("SCORING KEY (constant): Positive trait -> Score = roundup(Sten/2): 1-2=1 · 3-4=2 · 5-6=3 · 7-8=4 · 9-10=5.  "
        ...
        ("SCORING KEY: the percentile is banded to 1-5 using the editable Percentile band "
         "table to the right of this sheet (default: even fifths)." if pct_mode else
    ...
    blk,blk_names=_online_block(ws,name,dims,online_paste,pct_mode)
    ...
    metric_lbl=("Percentile" if pct_mode else "Sten")
    heads=pre+[f"Mapped {name} Trait"]+(["Direction"] if has_dir else [])+ \
          rub_heads+[f"{metric_lbl} (auto)" if online_paste else metric_lbl,
                     "Score (1-5)","Competency Avg"]
    ...
    # Percentile banding table, parked two columns clear of the data table so it can never
    # collide with the merged key/instruction rows or the header row.
    pf=pt=pr=None
    PCT_R0,PCT_R1=4,8                     # the five band rows, clear of ONLINE_HR (9)
    if pct_mode:
        L=openpyxl.utils.get_column_letter
        pf,pt,pr=(L(len(cols)+2),L(len(cols)+3),L(len(cols)+4))
        ws[f"{pf}2"]="Percentile band"
        ws[f"{pf}2"].font=F_HDR; ws[f"{pf}2"].fill=C_HDR
        ws.merge_cells(f"{pf}2:{pr}2"); ws[f"{pf}2"].alignment=A_WC
        for c,lab in zip((pf,pt,pr),("From","To","Rating")):
            cell=ws[f"{c}3"]; cell.value=lab; cell.font=F_B; cell.fill=C_DIMBG
            cell.alignment=A_CTR; cell.border=BD
        for k,(lo,hi,rt) in enumerate(PCT_BANDS_DEFAULT):
            r=PCT_R0+k
            for c,v in zip((pf,pt,pr),(lo,hi,rt)):
                cell=ws[f"{c}{r}"]; cell.value=v; cell.font=F_N
                cell.alignment=A_CTR; cell.border=BD
                cell.fill=(C_IN if c in (pf,pt) else C_LOCK)
                if c in (pf,pt): cell.protection=Protection(locked=False)
        note=ws[f"{pf}10"]
        note.value="Edit From/To to re-cut the bands."; note.font=F_IT
        ws.merge_cells(f"{pf}10:{pr}10")
        for c in (pf,pt,pr): ws.column_dimensions[c].width=12
        ...
        for k in range(ge-gs+1):
            ...
            if k<slots:
                ...
                if has_dir:
                    ...
                    # read the rating off the editable band table (From/To/Rating) written
                    # to the right of the block; 0 means the bands don't cover this value
                    _b0,_b1=PCT_R0,PCT_R1
                    _sp=(f'SUMPRODUCT(({sten_col}{r}>=${pf}${_b0}:${pf}${_b1})*'
                         f'({sten_col}{r}<=${pt}${_b0}:${pt}${_b1})*${pr}${_b0}:${pr}${_b1})')
                    ws[f"{score_col}{r}"]=(f'=IF({sten_col}{r}="","",'
                                           f'IF({_sp}=0,"",{_sp}))')
                else:
...
def _build_consultant_paste(wb):
    """Freeform paste sheet for consultants: group row 2, headers row 3, data from row 4.
    Onlines Data pulls every column by header name, so column order never matters.
    Row 1 carries the instructions and a live checker (stamped by _stamp_paste_checker
    once Onlines Data exists and the trait count is known)."""
    ws=wb.create_sheet(CONSULT_SHEET)
    ws["A1"]="Paste something below to see the header check."
    ws["A1"].font=F_B; ws.merge_cells("A1:C1"); ws["A1"].alignment=A_WV
    ws["A1"].fill=C_LEG; ws["A1"].border=BD
    ws["D1"]=("Paste the score extract here starting in cell A2 - group row, trait header row and "
              ...
              "Headers are matched by NAME, and a header that merely contains the trait name is "
              "matched too - so \"OPQ32-Persuasive-Sten\" finds Persuasive on its own.\n"
              "SOME NAMES APPEAR TWICE and are never guessed: Status is also an identity column, "
              "Recognition sits inside Personal Recognition, and Verify's PERCENTILE and STEN "
              "columns share a trait name. When that happens the column is left blank and "
              "flagged. Open Onlines Data - RED means no header matched, AMBER means more than "
              "one did - then retype that header here so it exactly matches the trait name in "
              "row 3 of Onlines Data.")
    ws["D1"].font=F_IT; ws.merge_cells("D1:R1"); ws["D1"].alignment=A_WV
...
def _stamp_paste_checker(wb,first_col,last_col,n_traits):
    """Live 'x of y headers matched' line on the Consultant Paste sheet.

    Counts the Onlines Data helper row: a numeric entry means that trait found its column.
    Anything missing is split into 'ambiguous' (more than one header contains the name, so
    it was deliberately not guessed) and 'not found', because the fix differs."""
    if n_traits<=0:
        return
    ws=wb[CONSULT_SHEET]
    helper=f"'{OD_SHEET}'!${first_col}${OD_HELPER_ROW}:${last_col}${OD_HELPER_ROW}"
    names=f"'{OD_SHEET}'!${first_col}$3:${last_col}$3"
    cp_head=f"'{CONSULT_SHEET}'!$A${CP_HEADER_ROW}:${CP_LAST_COL}${CP_HEADER_ROW}"
    amb=f'SUMPRODUCT(({helper}="")*(COUNTIF({cp_head},"*"&{names}&"*")>1))'
    ws["A1"]=(f'=IF(COUNTA({cp_head})=0,"Nothing pasted yet.",'
              f'"Headers matched: "&COUNT({helper})&" of {n_traits}."'
              f'&IF(COUNT({helper})={n_traits}," All found.",'
              f'" "&({n_traits}-COUNT({helper}))&" to fix"'
              f'&IF({amb}>0," ("&{amb}&" ambiguous)","")&" - see Onlines Data."))')
    from openpyxl.formatting.rule import FormulaRule
    ws.conditional_formatting.add("A1", FormulaRule(
        formula=[f'AND(COUNTA({cp_head})>0,COUNT({helper})<{n_traits})'],
        fill=C_BAD, stopIfTrue=True))
    ws.conditional_formatting.add("A1", FormulaRule(
        formula=[f'AND(COUNTA({cp_head})>0,COUNT({helper})={n_traits})'],
        fill=C_OK, stopIfTrue=True))
...
def _build_onlines_data(wb,sjt_variant=DEFAULT_SJT_VARIANT,selected_online=None,
    ...
    all_groups={"MQ":("MQ.M5 Profile",MQ_DIMS),
                ...
                # only the metric the consultant chose; the Overall Score is not mapped
                "Verify":(VERIFY_GROUP,VERIFY_DIMS)}
    ...
    for gname,dims in groups:
        ...
        for d in dims:
            ...
            ws[f"{col}2"]=raw_header(d,verify_metric)      # exact export header (lookup key 1)
    ...
    for cidx,h in enumerate(headers,start=1):
        ...
        if cidx<=len(ident):
            ...
            # but in the trait row when it doesn't — so search both. Exact only: loosening
            # this is what would let an identity column be mistaken for a trait.
            hcell.value=(f'=IFERROR(MATCH({col}$3,{cp_head},0),'
                         f'IFERROR(MATCH({col}$3,{cp_grp},0),""))')
        else:
            # Traits, in order of confidence:
            #   1. the exact export header (row 2) - editable, so a client's own naming works
            #   2. the exact clean trait name (row 3)
            #   3. a header that CONTAINS the trait name, but only when exactly one does
            # Stage 3 is what makes "OPQ32-Persuasive-Sten" resolve on its own. The COUNTIF
            # guard is what stops it guessing: "Recognition" sits inside SJT's "Personal
            # Recognition", and a Verify trait appears in both the PERCENTILE and STEN
            # columns. Two or more hits -> leave blank and let the checker flag it.
            # Deliberately never searches the group row: see the identity note above.
            #
            # STAGE 2 IS SKIPPED for a trait whose name is also an identity column name -
            # MQ's "Status". When an export leaves identity names in the TRAIT row, an exact
            # match on "Status" lands on the identity column and pulls "Complete" into a sten.
            # Such a trait must resolve on its full export header or be refused by stage 3.
            collides = str(h).strip().lower() in {i.lower() for i in OD_IDENTITY}
            stage2=("" if collides else
                    f'IFERROR(MATCH({col}$3,{cp_head},0),')
            close2=("" if collides else ")")
            hcell.value=(f'=IFERROR(MATCH({col}$2,{cp_head},0),'
                         f'{stage2}'
                         f'IF(COUNTIF({cp_head},"*"&{col}$3&"*")=1,'
                         f'MATCH("*"&{col}$3&"*",{cp_head},0),"")'
                         f'{close2})')
        hcell.font=F_IT
    ...
    # Pinpoint the trouble columns on the header rows themselves, so a failure is visible
    # rather than showing up later as a mysteriously blank sten.
    #   red   = no header in the paste matched this trait
    #   amber = more than one header contains it, so it was NOT guessed - retype that header
    if col_idx>len(ident)+1:
        from openpyxl.formatting.rule import FormulaRule
        f=openpyxl.utils.get_column_letter(len(ident)+1)
        l=openpyxl.utils.get_column_letter(col_idx-1)
        rng=f"{f}2:{l}3"
        ws.conditional_formatting.add(rng, FormulaRule(
            formula=[f'AND({f}${OD_HELPER_ROW}="",COUNTIF({cp_head},"*"&{f}$3&"*")>1)'],
            fill=C_WARN, stopIfTrue=True))
        ws.conditional_formatting.add(rng, FormulaRule(
            formula=[f'{f}${OD_HELPER_ROW}=""'], fill=C_BAD, stopIfTrue=True))
...
"1. To be written in 'You'.\n"
"2. Mention strengths of the person (avoid replication of the behavioural indicators or "
"OPQ statements).\n"
"3. Highlight the area of development such that it is (a) specific (b) actionable "
"(c) constructive, it does not sound prescriptive (avoid 'should') and inspires the "
"participant (e.g. it would be beneficial, it could, try to, you may).\n"
"4. Record observations for these areas to justify being strength/areas of development "
...
"8. At least 1 Strength area (only if Rating 3-5) and at least 1 Development Area "
"(Rating 3 can be a relative area of development if no 1-2).")
...
def _build_tchart(wb,framework,meta=None):
    ...
    # Excel does not auto-fit a merged, wrapped cell, so the block is given headroom:
    # editing the guidelines later must not silently clip the last line.
    for r in range(2,13):
...
def build_workbook(framework, selected_tools, sjt_variant=DEFAULT_SJT_VARIANT, tool_comps=None,
    ...
    if verify_metric not in VERIFY_METRICS: verify_metric=DEFAULT_VERIFY_METRIC
    ...
    if has_online and online_paste:
        ...
        _build_onlines_data(wb,sjt_variant,selected_online,verify_metric)
        # the checker needs the trait span, which depends on which tools were picked
        n_tr=0
        for t in selected_online:
            if t=="OPQ":      n_tr+=len(OPQ_DIMS)+1          # + Consistency Measure
            elif t=="MQ":     n_tr+=len(MQ_DIMS)
            elif t=="SJT":    n_tr+=len(SJT_VARIANTS[sjt_variant]["dims"])
            elif t=="Verify": n_tr+=len(VERIFY_DIMS)
        first=openpyxl.utils.get_column_letter(len(OD_IDENTITY)+1)
        last=openpyxl.utils.get_column_letter(len(OD_IDENTITY)+n_tr)
        _stamp_paste_checker(wb,first,last,n_tr)
    for name in selected:
        ...
        if spec["kind"]=="online":
            ...
            _build_online(wb,name,dims,filt,mq_mode,online_paste,fmeta,verify_metric)
