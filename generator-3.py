...
TOOLS = {
    "OPQ":        {"kind":"online","dims":OPQ_DIMS,  "score_col":"E","family":"online"},
    "MQ":         {"kind":"online","dims":MQ_DIMS,   "score_col":"E","family":"online"},
    "SJT":        {"kind":"online","dims":SJT_SCALES,"score_col":"E","family":"online"},
...
CANONICAL = ["OPQ","MQ","SJT","Case Study","Role Play","BEI"]
...
def _build_online(wb,name,dims,framework):
    ...
    _title(ws,f"{name}  —  online psychometric",
        ...
        "Negative trait -> inverse: 1-2=5 · 3-4=4 · 5-6=3 · 7-8=2 · 9-10=1.","G")
    ws["A4"]=("Assessor pastes stens in the blue block on the right. Consultant maps up to "
              f"{TRAIT_SLOTS} traits per competency (any behaviour row in the block), sets Direction "
              "(Positive by default), and Sten, Score and the Competency Average fill automatically.")
    ws["A4"].font=F_IT; ws.merge_cells("A4:G4"); ws["A4"].alignment=A_WV
    heads=["Competency","Behaviour",f"Mapped {name} Trait","Direction","Sten","Score (1-5)","Competency Avg"]
    cols=["A","B","C","D","E","F","G"]
    for c,h in zip(cols,heads): ws[f"{c}{HR}"]=h
    _hdr(ws,HR,cols)
    # paste-stens block on the right
    ws[f"I{HR}"]=f"{name} Dimension"; ws[f"J{HR}"]="Paste Sten"; _hdr(ws,HR,["I","J"])
    for i,dim in enumerate(dims):
        r=FB+i
        ws[f"I{r}"]=dim; ws[f"I{r}"].font=F_N; ws[f"I{r}"].border=BD; ws[f"I{r}"].fill=C_DIMBG
        ws[f"J{r}"]=""; ws[f"J{r}"].fill=C_PASTE; ws[f"J{r}"].border=BD; ws[f"J{r}"].alignment=A_CTR
    dim_last=FB+len(dims)-1
    geo=_block_geometry(framework)
    for comp,behs,gs,ge in geo:
        for k in range(ge-gs+1):
            r=gs+k
            # competency + behaviour columns: unchanged look (comp repeated beside each behaviour)
            if k<len(behs):
                ws[f"A{r}"]=comp; ws[f"B{r}"]=behs[k]
            # trait slots: first TRAIT_SLOTS rows of the block
            if k<TRAIT_SLOTS:
                ws[f"C{r}"]=""; ws[f"C{r}"].fill=C_IN
                ws[f"D{r}"]="Positive"; ws[f"D{r}"].fill=C_IN
                ws[f"E{r}"]=f'=IFERROR(INDEX($J${FB}:$J${dim_last},MATCH(C{r},$I${FB}:$I${dim_last},0)),"")'
                ws[f"E{r}"].fill=C_LOCK
                ws[f"F{r}"]=(f'=IF(E{r}="","",IF(D{r}="Negative",6-ROUNDUP(E{r}/2,0),ROUNDUP(E{r}/2,0)))')
                ws[f"F{r}"].fill=C_LOCK
            _cellsetup(ws,r,cols)
            for cc in ["D","E","F"]: ws[f"{cc}{r}"].alignment=A_CTR
        # competency average box (merged down the block) -> feeds Detailed Integration
        g=ws[f"G{gs}"]
        g.value=f'=IFERROR(ROUND(AVERAGE(F{gs}:F{ge}),0),"")'
        g.fill=C_LOCK; g.font=F_B; g.alignment=A_CTR
        if ge>gs: ws.merge_cells(f"G{gs}:G{ge}")
        for rr in range(gs,ge+1): ws[f"G{rr}"].border=BD
    last=geo[-1][3]
    dv=DataValidation(type="list",formula1=f"=$I${FB}:$I${dim_last}",allow_blank=True)
    ...
    for c,w in zip(["A","B","C","D","E","F","G","H","I","J"],[20,40,24,11,7,10,12,3,24,11]):
...
def _build_di(wb,selected,framework):
    ...
    for comp,items in groupby(framework,key=lambda x:x[0]):
        ...
        for k,t in enumerate(tool_cols):
            ...
            dcol=cols[4+k]
            cell=di[f"{dcol}{gs}"]
            cell.value=f'=IF({_ref(t)}!G{blk_start}="","",{_ref(t)}!G{blk_start})'
