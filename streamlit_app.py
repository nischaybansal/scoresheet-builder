...
selected = [t for t, on in [("OPQ", opq), ("MQ", mq), ("SJT", sjt),
                            ("Case Study", cs), ("Role Play", rp), ("BEI", bei)] if on]

# ---------------- STEP 3: generate ----------------
st.markdown('<div class="sb-step">3 · Generate</div>', unsafe_allow_html=True)
dc_name = st.text_input("File name (optional)", placeholder="e.g. Individual Feedback Scoresheet - Simple Sample")
...
if st.button("Build scoresheet", type="primary"):
    ...
    if not selected:
        ...
        try:
            ...
            wb = g.build_workbook(framework, selected)
            buf = io.BytesIO()
            wb.save(buf)
