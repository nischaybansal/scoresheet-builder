...
if fw_error:
    ...
    for c, _ in framework:
        if c not in comps_list:
            comps_list.append(c)
    extras=[]
    if (fw_meta or {}).get("themes"): extras.append(fw_meta["theme_label"])
    if (fw_meta or {}).get("levels"): extras.append(fw_meta["level_label"])
    st.success(f"Framework read: {len(framework)} behaviours across {len(comps_list)} competencies."
               + (f" Also picked up: {', '.join(extras)}." if extras else ""))
...
with col_on:
    st.markdown('<div class="sb-online">Online · psychometrics</div>', unsafe_allow_html=True)
    ...
    if mq:
        ...
        mq_mode = "5-point"
    if mq and mq_mode == "low-high":
        # standalone motivational profile - not mapped to competencies, absent from integration
        st.caption("Low / High MQ is a standalone profile: no competency mapping, and it does "
                   "not appear in the Detailed Integration or Integration sheets.")
        mq_c = None
    else:
...
with col_off:
    ...
    brp = st.checkbox("Business Role Play — Rubric anchors")
    brp_c = comp_picker("Business Role Play", brp)
    crp = st.checkbox("Coaching Role Play — Rubric anchors")
    crp_c = comp_picker("Coaching Role Play", crp)
    bei = st.checkbox("BEI — Interview questions")
    bei_c = comp_picker("BEI", bei)

online_paste = True
if any([opq, mq, sjt, verify]):
    st.markdown("**How do the online scores get in?**")
    _mode = st.radio(
        "Online scores", ["Consultant pastes the score extract", "Assessors type the stens"],
        horizontal=True, label_visibility="collapsed",
        help="Pasting adds the Consultant Paste and Onlines Data sheets and fills the stens "
             "automatically. Typing removes both sheets and the stens are entered on each "
             "tool sheet.")
    online_paste = _mode.startswith("Consultant")
    st.caption("Adds a Consultant Paste sheet and an Onlines Data sheet; pick the participant "
               "in Detailed Integration and every sten fills itself."
               if online_paste else
               "No Consultant Paste or Onlines Data sheet — the sten goes in on the tool sheet "
               "next to each mapped trait.")

show_summary = st.checkbox(
    "Add the competency summary table at the bottom of Detailed Integration", value=False,
...
selected = [t for t, on in [("OPQ", opq), ("MQ", mq), ("SJT", sjt), ("Verify", verify),
                            ...
                            ("Business Role Play", brp),
                            ("Coaching Role Play", crp), ("BEI", bei)] if on]
tool_comps = {t: c for t, c in [("OPQ", opq_c), ("MQ", mq_c), ("SJT", sjt_c), ("Verify", verify_c),
                                ...
                                ("Business Role Play", brp_c),
                                ("Coaching Role Play", crp_c), ("BEI", bei_c)] if c is not None}
