...
# --------------------------------------------------------------------------- helpers
COMP_BOX_MAX = 7      # competencies shown before the picker box starts scrolling
COMP_BOX_H = 250      # px height of that scrollable box
...
# The "comp_" prefix covers the per-competency checkboxes and the older "comps_" keys.
_sig = tuple(comps_list)
if st.session_state.get("_fw_sig") != _sig:
    for k in [k for k in list(st.session_state) if k.startswith("comp_")
              or k.startswith("comps_")]:
...
def comp_picker(tool, enabled):
    """Per-tool competency mapping, as a box of checkboxes that opens with the tool.

    Deliberately not a multiselect: every tick there is a rerun, which closes the dropdown,
    and the chips sit in the order they were picked rather than the framework's order. Ticking
    boxes leaves the list open and keeps the competencies in the order they were written.
    """
    if not (enabled and comps_list):
        return None

    keys = [f"comp_{tool}_{i}" for i in range(len(comps_list))]

    # The All / Clear buttons are rendered BEFORE the checkboxes on purpose: writing to a
    # widget's key is only safe while that widget has not yet been created in this run.
    head = st.columns([2.2, 1, 1])
    if head[1].button("All", key=f"all_{tool}", help=f"Select every competency for {tool}"):
        for k in keys:
            st.session_state[k] = True
    if head[2].button("Clear", key=f"clr_{tool}", help=f"Unselect every competency for {tool}"):
        for k in keys:
            st.session_state[k] = False

    # Counted after the buttons have applied, so the caption is never a run behind.
    n = sum(1 for k in keys if st.session_state.get(k, True))
    head[0].caption(f"Competencies · {n} of {len(comps_list)}")

    # A long framework gets a fixed-height, scrollable box so five ticked tools don't
    # stretch the page; a short one just sizes to its contents.
    box = (st.container(height=COMP_BOX_H, border=True) if len(comps_list) > COMP_BOX_MAX
           else st.container(border=True))
    picked = []
    with box:
        for comp, k in zip(comps_list, keys):
            if st.checkbox(comp, value=True, key=k):
                picked.append(comp)
    return picked
    ...
    for tool, blurb in ONLINE_TOOLS:
        ...
        if tool == "MQ":
            ...
            if on and mq_mode == "low-high":
                ...
                continue
        if tool == "Verify":
            verify_metric = (st.selectbox(
                "Verify maps to", ["Sten", "Percentile"], key="verify_metric",
                help="Which column of the Verify export feeds the score. Percentile is "
                     "banded to 1-5 on an editable table in the workbook (default: even "
                     "fifths).") if on else "Sten")
...
cfg_sig = repr((tuple(framework or ()), tuple(selected),
                sjt_variant, mq_mode, verify_metric, online_paste, show_summary,
...
if st.button("Build scoresheet", type="primary", disabled=bool(blockers)):
    ...
    try:
        wb = g.build_workbook(framework, selected, sjt_variant=sjt_variant,
                              ...
                              show_summary=show_summary, verify_metric=verify_metric)
        buf = io.BytesIO()
        wb.save(buf)
...
with st.sidebar:
    ...
    if chosen.get("Verify"):
        rows.append(("Verify", verify_metric))
    if any_online:
